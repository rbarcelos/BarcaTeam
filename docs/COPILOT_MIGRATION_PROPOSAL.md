...
**Phase 5 — WhatsApp bridge.** Rename `claude-ping/` → `whatsapp-bridge/`; repoint `ClaudeProcess` spawn from `claude -p` to `copilot -p` (or remove the chat-spawn feature, keep the WhatsApp send/receive MCP only). Update package name, README, tests.
...
1. **claude-ping / WhatsApp:** repoint to `copilot -p`, or strip the chat-spawn feature and keep only WhatsApp send/receive MCP? (Recommend: keep WhatsApp MCP, drop/repoint the Claude spawn.)
