---
name: git-workflow
description: Complete git conventions for capability implementation — cap branch lifecycle, subagent worktree operations, commit messages, PRs, and cleanup.
---
...
| Subagent branch | `agent/<agent-name>/<cap_slug>` | Each subagent | Isolated work branch, merged into cap |
| Hotfix | `fix/<short-description>` | Engineer | Quick fixes outside a capability |

Never commit directly to `main` or `cap/*`. Always work in a subagent branch.
...
main
 └── cap/<cap_slug>                        ← Lead creates, one per repo
      ├── agent/architect/<cap_slug>        ← architect subagent worktree
      ├── agent/senior-engineer/<cap_slug>  ← engineer subagent worktree
...
### CREATE — Cap Branch (Lead, before any subagent starts)
...
### CREATE — Subagent Worktrees (Lead, before invoking each teammate)

The lead creates a worktree for each Copilot subagent it is about to invoke, then passes the worktree path in the subagent prompt. Subagents do not create their own worktrees. Worktrees live under `<repo>/.worktrees/<name>/`; add `.worktrees/` to `.gitignore`.

```bash
# Run once per subagent per repo before invoking that subagent
REPO_ROOT=$(git -C "$REPO_DIR" rev-parse --show-toplevel)
...
### MERGE — Subagent Branch → Cap Branch (after subagent work is complete)
...
When two subagents need to edit the same file simultaneously:
1. **Stop** — do not edit the file.
2. **Return a blocked handoff to the lead** with: which file, which subagents, what changes each needs.
3. Lead assigns ownership — one agent proceeds, the other waits.
4. Waiting subagent rebases after first agent's commit is merged: `git rebase origin/cap/<cap_slug>`
5. If rebase conflict is unresolvable, escalate to the **architect** via the lead.
