...
description: Standard execution workflow for all Copilot coding subagents — worktree setup, planning, implementation, self-review, E2E gate, commit+merge, and handoff. Follow this in order for every capability.
---
...
## B) Worktree
Your worktree was created by the lead before you were invoked. Check your subagent prompt for `WORKTREE` and `BRANCH` — work exclusively in that path. Never work in the main repo directory.

If no worktree was provided, return a blocked response to the lead before touching any files.
...
- Use Copilot code search, `/lsp`, or an `explore` subagent for codebase investigation.
- Follow commit message conventions from **git-workflow**.

## D) Self-Review
Before requesting sign-off, use `/review` where appropriate and run through the **code-review-checklist** skill:
- Fix any Critical or Major issues found.
- Document Minor issues in the PR description.

## E) E2E Gate
- For UI flows, verify with Playwright MCP or the webapp-testing flow.
- For security-sensitive changes, run `/security-review`.
- For fast-moving external libraries, consult current docs through the docs-resolver agent or context7 MCP.
...
3. Run the **MERGE** operation from **git-workflow** — merge your subagent branch into `cap/<cap_slug>` and push.
