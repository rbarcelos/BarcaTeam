---
name: mock-to-production
description: 100% fidelity implementation of an HTML mock into production React/Tailwind components using audit, fix, verify, and final gate loops.
---
...
**Copilot subagent: ux-qa-tester** (reviewer role — never implements)
...
**Copilot subagent: ux-qa-tester** (reviewer role)
...
**Copilot subagent: senior-engineer** (multiple in parallel for independent components)
...
**Copilot subagent: ux-qa-tester** (same reviewer as Phase 2 — never the implementer)
...
10. **Parallel Copilot subagents, not serial rounds.** Fix multiple components simultaneously — one subagent per component. The bottleneck is verification, not implementation.
...
| Phase | Copilot subagent(s) | Role | Output |
|-------|----------|------|--------|
