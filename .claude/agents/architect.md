...
- **`docs-resolver` agent or Context7 MCP** — before prescribing a library/version, fetch the current docs. Pin the version in the ADR.
- **Copilot `/lsp`** — use for impact analysis when modifying shared types or interfaces.

## Handoffs
Follow the **team-handoff** skill protocol.
- Wait for the **PM** to complete PM_BRIEF.md before starting.
- The lead coordinates handoffs; provide a structured handoff for the **Senior Engineer** with tech prescriptions, contracts, and ADRs.
- Use the **code-review-checklist** skill when reviewing Engineer diffs.
- **Sign off** by returning the review verdict format to the lead.
